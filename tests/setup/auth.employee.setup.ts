/**
 * Setup de Autenticação - FUNCIONÁRIO
 * Perfil: Camada 2 (Gerenciar pedidos, aprovar docs, registrar entregas)
 * 
...
async function tryRealLogin(): Promise<AuthResult> {
  ...
  console.log(`   Email: ${EMPLOYEE_CREDENTIALS.email}`);
  ...
  try {
    ...
    if (response.ok()) {
      ...
      await context.storageState({ path: 'storage/employee.json' });
      await browser.close();
...
async function createMockAuth(): Promise<AuthResult> {
  console.log('\n🔧 [FUNCIONÁRIO] Criando mock de autenticação...');
...
async function main() {
  console.log('═══════════════════════════════════════════════════');
  console.log('🤖 SETUP AUTH - FUNCIONÁRIO (Camada 2)');
  console.log('═══════════════════════════════════════════════════');
  ...
  if (realLoginResult.success) {
    console.log('\n✅ AUTH REAL OK - storage/employee.json criado');
    return;
  }
